(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('dryhomecrmApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-dryhomecrmApp-error');
                var entityKey = httpResponse.headers('X-dryhomecrmApp-params');
                if (errorHeader) {
